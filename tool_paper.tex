...
windows, and independent confirmations, and imports examples into runnable benchmarks while recording
submitter labels as untrusted and deriving cases from independent redacted evidence cues; private
adopters can then run federated benchmark splits locally and publish only signed, k-thresholded
aggregate metrics.
Critically,
...
\begin{enumerate}[leftmargin=1.4em]
  ...
  severity calibration, a certificate-backed public evidence marketplace, and signed federated
  benchmark aggregates for private adopters---designed for artifact
...
certificate-obligation, public-data reproduction, and deterministic reputation checks pass, then import
published examples into runnable benchmark cases without trusting registry or artifact hazard labels. &
...
importer then reuses the published report, rehashes the artifact it actually reads, records both
registry and artifact \code{hazard\_class} values as untrusted submitter labels, derives labels from a
closed table of redacted snippet cues, and emits SQL fixtures plus ground truth with certificate,
evidence-hash, and artifact-hash provenance. The \gate{evidence-marketplace-gate} publishes two
redacted public-corpus examples, proves both have public-release eligibility and reviewable-or-better
gate reputation, imports both into a runnable benchmark manifest, validates and runs that manifest, and
then corrupts a copied certificate hash; the bad marketplace must be rejected while the tracked fixture
remains untouched.
...
2 public-release-eligible examples with reviewable gate reputation imported into runnable benchmarks; corrupted certificate rejected & \gate{evidence-marketplace-gate} \\
...
federated, deduplicated, archived, challenge-ready public evidence marketplace with explicit
