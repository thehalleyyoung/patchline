...
marketplace admits only redacted, certificate-backed hazard examples with clear licenses, verified
artifact hashes, explicit consent, reproducible commands, and a benchmark-import path that records
submitter labels as untrusted while deriving runnable cases from independent redacted evidence cues.
Critically,
...
hazard examples only after clear-license, consent, symlink-safe artifact-hash, certificate-obligation,
and public-data reproduction checks pass, then import published examples into runnable benchmark cases
without trusting registry or artifact hazard labels. & \code{marketplace.json},
...
if the declared SPDX license is accepted, the consent statement is explicit, all artifact paths stay
inside the registry after symlink resolution, every artifact hash recomputes, redaction is reviewed,
raw data is absent, and the certificate subject hash recomputes from normalized source, redaction,
artifact, obligation, license, and reproduction metadata. The benchmark importer then reuses the
published report, rehashes the artifact it actually reads, records both registry and artifact
\code{hazard\_class} values as untrusted submitter labels, derives labels from a closed table of
redacted snippet cues, and emits SQL fixtures plus ground truth with certificate, evidence-hash, and
artifact-hash provenance. The \gate{evidence-marketplace-gate} publishes two redacted public-corpus
examples, imports both into a runnable benchmark manifest, validates and runs that manifest, and then
...
2 certificate-backed examples imported into runnable benchmarks; corrupted certificate rejected & \gate{evidence-marketplace-gate} \\
...
public evidence marketplace with licenses, bounties, minimization, and governance; engine-version
semantics for production databases, locks, replication, partitioning, and online schema change;
