...
prevalence accounting, and imports only prevalence representatives into runnable benchmarks while
recording submitter labels as untrusted and deriving cases from independent redacted evidence cues;
...
checks pass, then import only prevalence representatives into runnable benchmark cases without trusting
registry or artifact hazard labels. &
\code{marketplace.json},
\code{marketplace-import.json}, generated manifest/ground truth \\
...
SQL fixtures plus ground truth with certificate, evidence-hash, and artifact-hash provenance. The
\gate{evidence-marketplace-gate} publishes two redacted public-corpus examples, proves both have
public-release eligibility, reviewable-or-better gate reputation, and duplicate-collapsed prevalence
weight one, imports both into a runnable benchmark manifest, validates and runs that manifest, and then
corrupts a copied certificate hash; the bad marketplace must be rejected while the tracked fixture
remains untouched.
...
2 public-release-eligible examples with reviewable gate reputation and duplicate-collapsed prevalence imported into runnable benchmarks; corrupted certificate rejected & \gate{evidence-marketplace-gate} \\
...
federated, archived, challenge-ready public evidence marketplace with explicit
