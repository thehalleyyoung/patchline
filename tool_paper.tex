...
ecosystems, versioned database semantics, lock-mode simulations, online-schema-change adapters,
...
\begin{enumerate}[leftmargin=1.4em]
  ...
  versioned database semantics, online-schema-change adapters, query-plan regression preflight, data-volume-aware runtime estimates, partition/shard routing semantics, rollback feasibility, and lock-mode conflict simulations
...
SQL on ClickHouse while actually launching background mutations. The same surface now emits a
...
unsupported-engine negative control.
...
Database engine semantics & \code{db\_semantics}, \code{engine\_profile}, \code{lock\_simulation}, \code{online\_schema\_change}, \code{replication\_lag\_risk}, \code{runtime\_estimate}, \code{partition\_sharding}, \code{rollback\_feasibility} & 8-engine versioned fixture + lock-mode simulator + online-schema-change adapter gate + replication-lag risk gate + data-volume runtime gate + partition/sharding semantics gate + rollback-feasibility gate + unsupported-engine negative control \\
