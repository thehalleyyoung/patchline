...
ecosystems, versioned database semantics with computed unsafe counter-profile controls, lock-mode simulations, online-schema-change adapters,
...
\begin{enumerate}[leftmargin=1.4em]
  ...
  versioned database semantics with engine/version negative controls, online-schema-change adapters, query-plan regression preflight, data-volume-aware runtime estimates, partition/shard routing semantics, rollback feasibility, and lock-mode conflict simulations
...
unsupported-engine negative control and the computed unsafe counter-profile controls.
...
Database engine semantics & \code{db\_semantics}, \code{engine\_profile}, \code{lock\_simulation}, \code{online\_schema\_change}, \code{replication\_lag\_risk}, \code{runtime\_estimate}, \code{partition\_sharding}, \code{rollback\_feasibility}, \code{engine\_negative\_control} & 8-engine versioned fixture + lock-mode simulator + online-schema-change adapter gate + replication-lag risk gate + data-volume runtime gate + partition/sharding semantics gate + rollback-feasibility gate + unsafe counter-profile and unsupported-engine negative controls \\
