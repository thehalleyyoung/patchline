...

{
  ...
  echo
  echo "| Project slice | Ecosystem | Migration framework | Size class | Evidence types | Commit | Files | Facts | Links | Time signals | Risks | Generated artifacts | Covered risks | New high-risk SQL | Cache hit |"
  echo "| --- | --- | --- | --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |"
  jq -r '.slices[] | "| \(.label) (`\(.repo):\(.subpath)`) | \(.ecosystem) | \(.migration_framework) | \(.repo_size_class) | \(.available_evidence_types | join(", ")) | `\(.resolved_commit[0:12])` | \(.files_scanned) | \(.facts) | \(.linked_candidates) | \(.time_signals) | \(.ranked_risks) | \(.generated_artifacts) | \(.before_after_deltas.risks_with_generated_coverage) | \(.before_after_deltas.new_high_risk_sql) | \(.cache_hit) |"' "$OUT/slice-matrix.json"
} > "$OUT/slice-matrix.md"
