...
while IFS='|' read -r label repo subpath; do
  ...
  go run ./cmd/patchline repo analyze --github "$repo" --subpath "$subpath" --stages inventory,baseline,propose,compare,deep --proposal-kind all --budget files=15,lines=120,tokens=50000,changes=3 --no-llm --resume --out "$case_out/analyze" --json > "$case_out/analyze-resume.json"
