...
while IFS='|' read -r label repo subpath; do
  ...
  go run ./cmd/patchline repo analyze --github "$repo" --subpath "$subpath" --stages inventory,baseline,propose,compare,deep --proposal-kind all --budget files=15,lines=120,tokens=50000,changes=3 --redact --no-llm --out "$case_out/analyze" --json > "$case_out/analyze.json"
  jq -e '.input | startswith("[redacted:")' "$case_out/analyze/analysis-bundle/source.json" > /dev/null
  go run ./cmd/patchline repo analyze --github "$repo" --subpath "$subpath" --stages inventory,baseline,propose,compare,deep --proposal-kind all --budget files=15,lines=120,tokens=50000,changes=3 --redact --no-llm --resume --out "$case_out/analyze" --json > "$case_out/analyze-resume.json"
