...

{
  ...
  echo
  echo "| Project slice | Ecosystem | Migration framework | Size class | Evidence types | Commit | Files | Facts | Grep-only matches | SQL-only risks | Ranked risks | Risk delta vs grep | Risk delta vs SQL-only | Evidence-link delta vs SQL-only | Links | Time signals | Generated artifacts | Covered risks | New high-risk SQL | Cache hit |"
  echo "| --- | --- | --- | --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |"
  jq -r '.slices[] | "| \(.label) (`\(.repo):\(.subpath)`) | \(.ecosystem) | \(.migration_framework) | \(.repo_size_class) | \(.available_evidence_types | join(", ")) | `\(.resolved_commit[0:12])` | \(.files_scanned) | \(.facts) | \(.grep_only_comparison.grep_only_matches) | \(.sql_only_comparison.sql_only_ranked_risks) | \(.ranked_risks) | \(.grep_only_comparison.ranked_risk_delta) | \(.sql_only_comparison.ranked_risk_delta) | \(.sql_only_comparison.evidence_link_delta) | \(.linked_candidates) | \(.time_signals) | \(.generated_artifacts) | \(.before_after_deltas.risks_with_generated_coverage) | \(.before_after_deltas.new_high_risk_sql) | \(.cache_hit) |"' "$OUT/slice-matrix.json"
} > "$OUT/slice-matrix.md"
