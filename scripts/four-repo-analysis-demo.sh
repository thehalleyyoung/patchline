...
while IFS='|' read -r label repo subpath; do
  ...
  jq \
    ...
    '. + {label: $label, repo: $repo, subpath: $subpath, case_dir: "'"$case_out"'", llm_command_proof: {generator: $llm[0].generator, deterministic_only: $llm[0].deterministic_only, generated_files: ($llm[0].generated_files | length), intervention_loops: $llmcompare[0].summary.intervention_loops, checks_failed: $llmcompare[0].summary.patchline_checks_failed}, cache_proof: {hit: $cache[0].source.cache_hit, archive_hash: $cache[0].source.archive_hash, resolved_commit: $cache[0].source.resolved_commit}, analyze_proof: {stages: $analyze[0].stages, ranked_risks: $analyze[0].summary.ranked_risks, generated_files: $analyze[0].summary.generated_files, intervention_loops: $analyze[0].summary.intervention_loops, hash: $analyze[0].hash}, structured_proof: {files: $structured[0].files_scanned, field_evidence: ($structured[0].summary_by_category.field_evidence // 0)}}' \
