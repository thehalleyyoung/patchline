...
while IFS='|' read -r label repo subpath; do
  ...
  go run ./cmd/patchline repo analyze --github "$repo" --subpath "$subpath" --stages inventory,baseline,propose,compare,deep --proposal-kind all --budget files=15,lines=120,tokens=50000,changes=3 --no-llm --out "$case_out/analyze" --json > "$case_out/analyze.json"
  ...
  jq \
    ...
    '. + {label: $label, repo: $repo, subpath: $subpath, case_dir: "'"$case_out"'", llm_command_proof: {generator: $llm[0].generator, deterministic_only: $llm[0].deterministic_only, scope_budget: ($llm[0].scope_budget.raw // ""), generated_files: ($llm[0].generated_files | length), intervention_loops: $llmcompare[0].summary.intervention_loops, checks_failed: $llmcompare[0].summary.patchline_checks_failed}, cache_proof: {hit: $cache[0].source.cache_hit, archive_hash: $cache[0].source.archive_hash, resolved_commit: $cache[0].source.resolved_commit}, analyze_proof: {stages: $analyze[0].stages, scope_budget: ($analyze[0].summary.scope_budget // ""), ranked_risks: $analyze[0].summary.ranked_risks, generated_files: $analyze[0].summary.generated_files, intervention_loops: $analyze[0].summary.intervention_loops, bundle_files: $bundle_files, hash: $analyze[0].hash}, structured_proof: {files: $structured[0].files_scanned, field_evidence: ($structured[0].summary_by_category.field_evidence // 0)}}' \
