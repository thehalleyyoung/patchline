...

{
  ...
  echo
  echo "| Project slice | Ecosystem | Migration framework | Size class | Evidence types | Commit | Files | Facts | Grep-only matches | SQL-only risks | Identifier-only links | Temporal/date-only links | Ranked risks | Risk delta vs grep | Risk delta vs SQL-only | Evidence-link delta vs identifier-only | Evidence-link delta vs temporal-only | Identifier-link delta vs temporal-only | Links | Time signals | Generated artifacts | Fact prompt differs | Fact output differs | Covered risks | New high-risk SQL | Cache hit |"
  echo "| --- | --- | --- | --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- | --- | ---: | ---: | --- |"
  jq -r '.slices[] | "| \(.label) (`\(.repo):\(.subpath)`) | \(.ecosystem) | \(.migration_framework) | \(.repo_size_class) | \(.available_evidence_types | join(", ")) | `\(.resolved_commit[0:12])` | \(.files_scanned) | \(.facts) | \(.grep_only_comparison.grep_only_matches) | \(.sql_only_comparison.sql_only_ranked_risks) | \(.identifier_only_comparison.identifier_only_links) | \(.identifier_only_comparison.temporal_or_date_only_links) | \(.ranked_risks) | \(.grep_only_comparison.ranked_risk_delta) | \(.sql_only_comparison.ranked_risk_delta) | \(.identifier_only_comparison.evidence_link_delta) | \(.temporal_only_comparison.evidence_link_delta) | \(.temporal_only_comparison.identifier_link_delta) | \(.linked_candidates) | \(.time_signals) | \(.generated_artifacts) | \(.generation_comparison.fact_grounded_prompt_hash != .generation_comparison.without_facts_prompt_hash) | \(.generation_comparison.fact_grounded_output_hash != .generation_comparison.without_facts_output_hash) | \(.before_after_deltas.risks_with_generated_coverage) | \(.before_after_deltas.new_high_risk_sql) | \(.cache_hit) |"' "$OUT/slice-matrix.json"
} > "$OUT/slice-matrix.md"
