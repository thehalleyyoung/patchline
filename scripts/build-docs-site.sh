...
jq -n \
  ...
  --slurpfile manyMatrix examples/public-repo-many-matrix-results.json \
...
cp examples/public-repo-many-matrix-results.json "$SITE/artifacts/public-repo-many-matrix-results.json"
...
many_matrix_slices="$(jq '.many_repo_matrix.summary.slices' "$SITE/artifacts/public-evidence.json")"
many_matrix_repos="$(jq '.many_repo_matrix.summary.unique_repos' "$SITE/artifacts/public-evidence.json")"
many_matrix_ecosystems="$(jq '.many_repo_matrix.summary.ecosystems' "$SITE/artifacts/public-evidence.json")"
many_matrix_files="$(jq '.many_repo_matrix.summary.total_files_scanned' "$SITE/artifacts/public-evidence.json")"
many_matrix_risks="$(jq '.many_repo_matrix.summary.total_ranked_risks' "$SITE/artifacts/public-evidence.json")"
...
<main>
<section class="card"><h2>Real public-repo output</h2><p>This site build regenerated <code>$repo</code> <code>$subpath</code> at pinned commit <code>$ref</code>. Beyond the four-repo curated catalog, the larger public matrix has now been run on <strong>$many_matrix_slices</strong> public slices across <strong>$many_matrix_repos</strong> GitHub repos and <strong>$many_matrix_ecosystems</strong> ecosystems, with offline validation passing for every slice.</p><div class="grid"><div><div class="metric">$many_matrix_slices</div><p>public slices in the larger matrix</p></div><div><div class="metric">$many_matrix_repos</div><p>GitHub repos in that matrix</p></div><div><div class="metric">$many_matrix_files</div><p>public files scanned in the matrix</p></div><div><div class="metric">$many_matrix_risks</div><p>ranked risks in the matrix</p></div><div><div class="metric">$confirmed_public_incidents</div><p>confirmed public incident bug cases</p></div></div><p><a href="artifacts/public-demo.json">Download public-demo.json</a>, <a href="artifacts/public-evidence.json">public-evidence.json</a>, and <a href="artifacts/public-repo-many-matrix-results.json">public-repo-many-matrix-results.json</a>.</p></section>
<section class="card"><h2>Empirically shown in this repo</h2><p>The repository evidence currently demonstrates concrete public-repo coverage: Patchline ran end-to-end on a pinned public Lobsters migration slice; the larger matrix passed on <strong>$many_matrix_slices</strong> public slices across <strong>$many_matrix_repos</strong> GitHub repos including Flask, Bytebase, Django, Airflow, Lobsters, Prisma, TypeORM, dbt, Kubernetes examples, Protobuf, and MongoDB Prisma; the curated catalog additionally covers Forem, Bytebase, Mastodon, and Lobsters; benchmark reports contain confirmed public incident cases for GitLab 2017 primary data loss and GitHub 2018 split-brain write divergence; and the analyzer emits specific review surfaces such as broad writes, transaction/idempotency gaps, lock hazards, privacy hazards, provenance slices, symbolic checks, policy failures, and generated-artifact compare checks.</p></section>
...
cat > "$SITE/api/index.html" <<EOF
  <!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Public repo API manual - Patchline</title><link rel="stylesheet" href="../styles.css"></head><body><header><div class="hero"><div class="eyebrow">Public repo API manual</div><h1>Run Patchline on public repositories, inspect every stage, and know what happens next.</h1><p>Intended to work with any GitHub repo; tested on many.</p>$api_nav</div></header><main><section class="card"><h2>Tested on many public repos</h2><p>The API is not only backed by the four-repo curated catalog. The larger public matrix was run on <strong>$many_matrix_slices</strong> public slices across <strong>$many_matrix_repos</strong> GitHub repos and <strong>$many_matrix_ecosystems</strong> ecosystems, scanning <strong>$many_matrix_files</strong> files and ranking <strong>$many_matrix_risks</strong> risks with offline validation passing for every slice.</p><p>The matrix includes Flask/Python, raw Bytebase SQL migrations, Django migrations, Airflow Alembic migrations, Rails migrations, Prisma, TypeORM, dbt SQL models, Kubernetes data-service manifests, Protobuf schemas, and MongoDB Prisma. See <a href="../artifacts/public-repo-many-matrix-results.json">public-repo-many-matrix-results.json</a>.</p></section><section class="card"><h2>API map</h2><table><tr><th>Need</th><th>API page</th><th>What happens</th></tr><tr><td>Download and pin a public source slice</td><td><a href="fetch.html">Fetch API</a></td><td>Writes source metadata, cache proof, archive hash, resolved commit, and scanned root.</td></tr><tr><td>Run the whole workflow in one command</td><td><a href="analyze.html">Analyze API</a></td><td>Fetches, inventories, intakes, ranks, proposes, compares, and writes an analysis bundle.</td></tr><tr><td>Inspect each stage separately</td><td><a href="staged-workflow.html">Staged workflow API</a></td><td>Runs inventory, baseline, playbook, propose, compare, and optional deep outputs as separate artifacts.</td></tr><tr><td>Generate review artifacts safely</td><td><a href="interventions.html">Intervention API</a></td><td>Writes quarantined tests/guards/explain/repair artifacts and re-checks them before review.</td></tr><tr><td>Share or replay evidence</td><td><a href="offline-redaction-ci.html">Offline/redaction/CI API</a></td><td>Uses cache-backed offline validation, redaction, SARIF, and CI helper output.</td></tr><tr><td>Know which files to inspect</td><td><a href="outputs.html">Output schema guide</a></td><td>Explains source, inventory, intake, baseline, proposal, compare, bundle, and site evidence JSON.</td></tr></table></section><section class="card"><h2>Five Lobsters ranked risks worth review</h2><p>These are not claimed as confirmed production bugs. They are five high-scoring findings from the pinned Lobsters migration analysis that I agree are real review risks because the source performs broad destructive or bulk data-changing work where rollback, batching, idempotency, or table-scope evidence matters.</p><table><tr><th>Risk</th><th>Score</th><th>Why I agree it is a real risk</th></tr><tr><td><code>risk:970ac4cfcd161507</code><br><code>20240717155412_hydrate_links.rb:12</code><br><code>code-path:delete</code> on <code>links</code></td><td>149 high</td><td>The rollback path deletes all link rows after the migration has recreated links from comments and stories. A broad <code>delete_all</code> rollback can erase unrelated or newly created link state, so it deserves explicit scope and recovery review.</td></tr><tr><td><code>risk:2ded136b72a75f38</code><br><code>20250208222418_add_edit_timestamps.rb:16</code><br><code>code-path:update</code> on <code>comments</code></td><td>145 high</td><td>The migration backfills every comment's edit timestamp from <code>updated_at</code>, then makes the column non-null. That is a full-table data rewrite tied to a constraint change; it should be reviewed for lock time, batching, retry safety, and rollback behavior.</td></tr><tr><td><code>risk:4bf8b05aa283adf3</code><br><code>20200807021926_memoize_score_not_upvotes.rb</code><br><code>update</code> on <code>stories</code></td><td>140 high</td><td>The migration recalculates story score and flag materializations from votes with unbounded SQL updates. Recomputing denormalized counters across a whole table is a legitimate risk because a bad join or interrupted run can leave user-visible ranking data inconsistent.</td></tr><tr><td><code>risk:93bcbce17738750e</code><br><code>20250124063340_add_last_comment_at_to_stories.rb</code><br><code>update</code> on <code>stories</code></td><td>140 high</td><td>The migration backfills <code>last_comment_at</code> using an aggregate over comments and then indexes the new field. This is a whole-table backfill driven by another table; reviewers should check join scope, null behavior for stories without comments, and operational impact.</td></tr><tr><td><code>risk:a2044c7b196a722c</code><br><code>20230823150648_materialize_confidence_order.rb</code><br><code>update</code> on <code>comments</code></td><td>140 high</td><td>The migration materializes a binary ordering key for every comment from confidence and id. Bulk recomputation of ordering keys affects presentation/ranking semantics, and the migration's rollback path should be checked carefully because the data shape is table-specific.</td></tr></table></section><section class="card"><h2>Evidence boundary</h2><p>Public-repo analysis finds review risks and evidence links; labeled benchmark reports are where this repo makes confirmed bug/incident claims. The manual keeps those separate so a reviewer can tell the difference between a real-repo warning and a confirmed ground-truth case.</p></section></main></body></html>
EOF
...
<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Public repository scenario - Patchline</title><link rel="stylesheet" href="../styles.css"></head><body><header><div class="hero"><div class="eyebrow">Scenario</div><h1>It runs on real repos, not Patchline-shaped fixtures.</h1>$scenario_nav</div></header><main><section class="card"><h2>Pinned public demo</h2><pre>go run ./cmd/patchline repo analyze --github $repo --ref $ref --subpath $subpath --stages inventory,baseline,propose,compare --proposal-kind all --budget files=6,lines=90,tokens=10000,changes=2 --no-llm --out results/generated/docs-site/analysis --json</pre><p>This exact site build regenerated the public demo with <strong>$files</strong> files, <strong>$risks</strong> ranked risks, and <strong>$generated</strong> generated review artifacts.</p></section><section class="card"><h2>Many-repo evidence</h2><p>This is not just four repos. The current larger matrix passed on <strong>$many_matrix_slices</strong> public slices across <strong>$many_matrix_repos</strong> GitHub repositories and <strong>$many_matrix_ecosystems</strong> ecosystems, scanning <strong>$many_matrix_files</strong> files and ranking <strong>$many_matrix_risks</strong> risks. Offline validation passed for all <strong>$many_matrix_slices</strong> slices, compare reported zero deterministic failures, and deterministic generation was used for every case.</p><p>The larger matrix includes Flask/Python, raw Bytebase SQL migrations, Django migrations, Airflow Alembic migrations, Rails migrations, Prisma, TypeORM, dbt SQL models, Kubernetes data-service manifests, Protobuf schemas, and MongoDB Prisma. The separate curated catalog covers <strong>$slice_count</strong> pinned real-repo slices across <strong>$slice_repos</strong> repositories: Forem, Bytebase, Mastodon, and Lobsters. The benchmark layer adds <strong>$public_migration_cases</strong> pinned public Bytebase migration cases, <strong>$public_incident_cases</strong> public incident cases, public-derived repair/archive boundaries, and <strong>$smoke_cases</strong> smoke cases.</p><p><a href="../artifacts/public-repo-many-matrix-results.json">public-repo-many-matrix-results.json</a> records the larger matrix. <a href="../artifacts/public-evidence.json">public-evidence.json</a> records the curated catalog, sampled adjudications, benchmark case counts, and the embedded matrix summary.</p></section><section class="card"><h2>Confirmed bug boundary</h2><p>Patchline does not claim every public-repo risk is a confirmed bug. Confirmed bug/incident claims are reserved for labeled ground-truth cases, including GitLab 2017 public primary-data-loss observations, GitHub 2018 public split-brain write-divergence observations, committed unsafe backfill fixtures, and pinned public migration-risk labels.</p></section></main></body></html>
EOF
