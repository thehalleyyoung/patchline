...
Run `governance-risk-register --spec examples/governance-risk-register.json` to quantify maintainership, funding, infrastructure, and benchmark-control concentration with hash-backed evidence, review cadence, rotation, and mitigation gates; reproduce it with `make governance-risk-register-gate`.
...
Run `feedback detector-deprecation --feedback <live-feedback> --spec examples/detector-deprecation-gate.json` to retire weak detectors only after source-free precision/burden evidence plus public notice, independent review, appeal time, and migration guidance; reproduce it with `make detector-deprecation-gate`.
...
Run `evidence-marketplace appeal --spec examples/evidence-marketplace/appeal-workflow.json` to resolve disputed findings with hash-bound preserved evidence, independent reviewer rationales, and final audit trails; reproduce it with `make evidence-appeal-workflow-gate`.
...
Run `make role-specific-tutorials-gate` for role-specific tutorials that compile app developer, DBA, SRE, security reviewer, and engineering-manager playbooks from real-code evidence, rejecting a generic tutorial; see [docs/role-specific-tutorials.md](docs/role-specific-tutorials.md).
...
Run `make live-feedback-ingestion-gate` for **source-free live feedback** ingestion from adopters: reviewer outcomes become k-anonymous aggregate calibration signals while source code, diffs, file paths, finding IDs, evidence hashes, adopter IDs, and salts are rejected or discarded; see [docs/live-feedback-ingestion.md](docs/live-feedback-ingestion.md).
Run `make drift-threshold-update-gate` for drift-aware calibration recommendations that stay advisory unless a hash-bound policy gate authorizes a separate candidate policy; see [docs/drift-threshold-update.md](docs/drift-threshold-update.md).
Run `make reviewer-counterfactual-log-gate` to reconstruct what previous Patchline releases would have recommended from source-free reviewer outcomes, with decile-boundary ambiguity and missed non-emissions handled conservatively; see [docs/reviewer-counterfactual-log.md](docs/reviewer-counterfactual-log.md).
Run `make safe-online-evaluation-gate`, `make adopter-active-learning-gate`, `make policy-freeze-gate`, `make live-calibration-monitor-gate`, `make feedback-retention-lifecycle-gate`, `make human-trust-regression-gate`, and `make live-learning-methodology-gate` for the live-learning loop: shadow detector evaluation, local-only active learning, incident freezes, decile drift alerts, retention proofs, trust regressions, and recall-without-over-reliance methodology.
...
Run `make verified-end-to-end-pipeline-gate` for a **machine-checked end-to-end** proof that the fetch-to-verdict pipeline preserves hazard semantics at every stage, rejecting an unsupported item; see [docs/verified-end-to-end-pipeline.md](docs/verified-end-to-end-pipeline.md).
Run `make proof-carrying-patch-bundle-gate` for **proof-carrying patch** bundles where every emitted patch ships a re-checkable safety witness, rejecting an unsupported item; see [docs/proof-carrying-patch-bundle.md](docs/proof-carrying-patch-bundle.md).
Run `make verified-dialect-roundtrip-gate` for a verified **dialect round-trip** proving DSL-to-SQL-to-DSL is identity-preserving across engines, rejecting an unsupported item; see [docs/verified-dialect-roundtrip.md](docs/verified-dialect-roundtrip.md).
Run `make mechanized-rollback-safety-gate` for a mechanized **rollback safety proof** that every generated down-migration inverts its up-migration, rejecting an unsupported item; see [docs/mechanized-rollback-safety.md](docs/mechanized-rollback-safety.md).
Run `make verified-online-migration-protocol-gate` for a verified **online-migration protocol** proving zero-downtime expand/contract preserves invariants, rejecting an unsupported item; see [docs/verified-online-migration-protocol.md](docs/verified-online-migration-protocol.md).
Run `make refinement-typed-dsl-gate` for a **refinement-typed DSL** where column constraints are checked statically before any emission, rejecting an unsupported item; see [docs/refinement-typed-dsl.md](docs/refinement-typed-dsl.md).
Run `make verified-constraint-solver-gate` for a **verified constraint solver** core proving every reported conflict is a real unsatisfiable constraint, rejecting an unsupported item; see [docs/verified-constraint-solver.md](docs/verified-constraint-solver.md).
Run `make mechanized-isolation-levels-gate` for a mechanized **isolation-level model** proving anomaly-freedom per declared transaction isolation, rejecting an unsupported item; see [docs/mechanized-isolation-levels.md](docs/mechanized-isolation-levels.md).
Run `make proof-minimization-engine-gate` for a **proof minimization** engine emitting the smallest checkable witness for each verdict, rejecting an unsupported item; see [docs/proof-minimization-engine.md](docs/proof-minimization-engine.md).
Run `make self-checking-certificate-checker-gate` for a **self-checking certificate** checker that verifies its own checker against a reference oracle, rejecting an unsupported item; see [docs/self-checking-certificate-checker.md](docs/self-checking-certificate-checker.md).
Run `make hundred-million-migration-index-gate` for a **hundred-million-migration** content-addressed index with reproducible queryable hazard statistics, rejecting an unsupported item; see [docs/hundred-million-migration-index.md](docs/hundred-million-migration-index.md).
Run `make continuous-corpus-refresh-stream-gate` for a **continuous corpus refresh** stream ingesting public commits with deduplicated incremental indexing, rejecting an unsupported item; see [docs/continuous-corpus-refresh-stream.md](docs/continuous-corpus-refresh-stream.md).
Run `make cross-language-hazard-atlas-gate` for a **cross-language hazard atlas** comparing prevalence across ORMs and engines with confidence intervals, rejecting an unsupported item; see [docs/cross-language-hazard-atlas.md](docs/cross-language-hazard-atlas.md).
Run `make longitudinal-hazard-trends-gate` for a longitudinal hazard-trends report tracking prevalence over years with changepoint detection, rejecting an unsupported item; see [docs/longitudinal-hazard-trends.md](docs/longitudinal-hazard-trends.md).
Run `make corpus-sampling-theory-gate` for a **provable sampling bound** certifying corpus estimates are within a stated error with given confidence, rejecting an unsupported item; see [docs/corpus-sampling-theory.md](docs/corpus-sampling-theory.md).
Run `make incremental-global-dedup-gate` for an **incremental global dedup** proving each unique migration is counted exactly once across shards, rejecting an unsupported item; see [docs/incremental-global-dedup.md](docs/incremental-global-dedup.md).
Run `make differentially-private-aggregation-gate` for a differentially-private federated aggregation sharing hazard counts with a formal privacy budget, rejecting an unsupported item; see [docs/differentially-private-aggregation.md](docs/differentially-private-aggregation.md).
Run `make carbon-aware-scheduler-gate` for a carbon-aware scheduler shifting large analyses to low-intensity windows with measured savings, rejecting an unsupported item; see [docs/carbon-aware-scheduler.md](docs/carbon-aware-scheduler.md).
Run `make throughput-per-dollar-frontier-gate` for a **throughput-per-dollar frontier** with a proven cost lower bound across instance families, rejecting an unsupported item; see [docs/throughput-per-dollar-frontier.md](docs/throughput-per-dollar-frontier.md).
Run `make replicable-corpus-api-gate` for a replicable public corpus API serving versioned, content-pinned hazard statistics, rejecting an unsupported item; see [docs/replicable-corpus-api.md](docs/replicable-corpus-api.md).
Run `make stepped-wedge-trial-gate` for a stepped-wedge cluster trial rolling out gating across teams with pooled effect estimates, rejecting an unsupported item; see [docs/stepped-wedge-trial.md](docs/stepped-wedge-trial.md).
Run `make synthetic-control-study-gate` for a synthetic-control study constructing a counterfactual adopter from donor organizations, rejecting an unsupported item; see [docs/synthetic-control-study.md](docs/synthetic-control-study.md).
Run `make doubly-robust-estimator-gate` for a **doubly-robust estimator** of gating's incident effect valid if either model is correct, rejecting an unsupported item; see [docs/doubly-robust-estimator.md](docs/doubly-robust-estimator.md).
Run `make negative-control-outcomes-gate` for a **negative-control outcomes** calibration detecting residual confounding in the causal design, rejecting an unsupported item; see [docs/negative-control-outcomes.md](docs/negative-control-outcomes.md).
Run `make partial-identification-bounds-gate` for **partial-identification bounds** giving worst-case effect intervals under minimal assumptions, rejecting an unsupported item; see [docs/partial-identification-bounds.md](docs/partial-identification-bounds.md).
Run `make incident-survival-analysis-gate` for an incident **survival analysis** estimating time-to-next-incident hazard ratios for adopters, rejecting an unsupported item; see [docs/incident-survival-analysis.md](docs/incident-survival-analysis.md).
Run `make prospective-power-analysis-gate` for a **prospective power analysis** fixing sample sizes before any outcome is observed, rejecting an unsupported item; see [docs/prospective-power-analysis.md](docs/prospective-power-analysis.md).
Run `make specification-multiverse-gate` for a specification **multiverse analysis** showing the effect is stable across analytic choices, rejecting an unsupported item; see [docs/specification-multiverse.md](docs/specification-multiverse.md).
Run `make placebo-gate-falsification-gate` for a placebo-gate falsification confirming a sham gate produces no spurious effect, rejecting an unsupported item; see [docs/placebo-gate-falsification.md](docs/placebo-gate-falsification.md).
Run `make external-validity-transport-gate` for an **external-validity transport** estimate re-weighting results to a target population, rejecting an unsupported item; see [docs/external-validity-transport.md](docs/external-validity-transport.md).
Run `make manski-attrition-bounds-gate` for Manski **attrition bounds** proving conclusions survive worst-case dropout, rejecting an unsupported item; see [docs/manski-attrition-bounds.md](docs/manski-attrition-bounds.md).
Run `make cost-benefit-decision-model-gate` for a **cost-benefit decision** model monetizing prevented incidents against reviewer time with intervals, rejecting an unsupported item; see [docs/cost-benefit-decision-model.md](docs/cost-benefit-decision-model.md).
Run `make causal-forest-heterogeneity-gate` for a causal-forest heterogeneity analysis identifying which teams benefit most from gating, rejecting an unsupported item; see [docs/causal-forest-heterogeneity.md](docs/causal-forest-heterogeneity.md).
Run `make external-meta-analysis-gate` for an external meta-analytic pooling of independent studies with heterogeneity diagnostics, rejecting an unsupported item; see [docs/external-meta-analysis.md](docs/external-meta-analysis.md).
Run `make confirmatory-preregistration-gate` for a confirmatory pre-registered trial with a frozen analysis plan and sealed hypotheses, rejecting an unsupported item; see [docs/confirmatory-preregistration.md](docs/confirmatory-preregistration.md).
Run `make autonomous-fleet-orchestrator-gate` for an **autonomous fleet** orchestrator coordinating many repair agents under a global safety budget, rejecting an unsupported item; see [docs/autonomous-fleet-orchestrator.md](docs/autonomous-fleet-orchestrator.md).
Run `make dependency-aware-sequencing-gate` for a **dependency-aware sequencing** planner ordering cross-service migrations with two-phase safety, rejecting an unsupported item; see [docs/dependency-aware-sequencing.md](docs/dependency-aware-sequencing.md).
Run `make runtime-policy-monitor-gate` for a **runtime policy monitor** proving the agent never violates a declared safety automaton, rejecting an unsupported item; see [docs/runtime-policy-monitor.md](docs/runtime-policy-monitor.md).
Run `make online-learning-guard-gate` for an online-learning safety guard freezing any update that would weaken a soundness invariant, rejecting an unsupported item; see [docs/online-learning-guard.md](docs/online-learning-guard.md).
Run `make agent-budget-market-gate` for an agent **budget market** allocating compute to highest-value repairs under a hard ceiling, rejecting an unsupported item; see [docs/agent-budget-market.md](docs/agent-budget-market.md).
Run `make staged-rollout-controller-gate` for a staged-rollout controller promoting agent changes only after canary gates pass, rejecting an unsupported item; see [docs/staged-rollout-controller.md](docs/staged-rollout-controller.md).
Run `make incident-auto-remediation-gate` for an incident **auto-remediation** loop that proposes and verifies a fix before any human is paged, rejecting an unsupported item; see [docs/incident-auto-remediation.md](docs/incident-auto-remediation.md).
Run `make multi-tenant-agent-isolation-gate` for a multi-tenant agent isolation proof guaranteeing no cross-tenant data or action leakage, rejecting an unsupported item; see [docs/multi-tenant-agent-isolation.md](docs/multi-tenant-agent-isolation.md).
Run `make signed-agent-actions-gate` for **signed agent actions** where every merge is cryptographically attributable and verifiable, rejecting an unsupported item; see [docs/signed-agent-actions.md](docs/signed-agent-actions.md).
Run `make counterfactual-regret-eval-gate` for a counterfactual-regret evaluation bounding how far agent decisions trail the optimal policy, rejecting an unsupported item; see [docs/counterfactual-regret-eval.md](docs/counterfactual-regret-eval.md).
Run `make faithful-agent-explanations-gate` for faithful agent explanations proven to reflect the actual decision, not a post-hoc rationalization, rejecting an unsupported item; see [docs/faithful-agent-explanations.md](docs/faithful-agent-explanations.md).
Run `make safe-interruptibility-gate` for a safe-interruptibility guarantee that pausing the agent always leaves a consistent schema, rejecting an unsupported item; see [docs/safe-interruptibility.md](docs/safe-interruptibility.md).
Run `make agent-liveness-guarantee-gate` for an agent **liveness guarantee** proving every accepted task eventually merges or escalates, rejecting an unsupported item; see [docs/agent-liveness-guarantee.md](docs/agent-liveness-guarantee.md).
Run `make fleet-slo-enforcer-gate` for a **fleet SLO** enforcer holding latency and error budgets across the whole agent fleet, rejecting an unsupported item; see [docs/fleet-slo-enforcer.md](docs/fleet-slo-enforcer.md).
Run `make autonomy-certification-suite-gate` for an **autonomy certification** suite scoring the fleet against published maturity criteria, rejecting an unsupported item; see [docs/autonomy-certification-suite.md](docs/autonomy-certification-suite.md).
Run `make iso-track-certificate-standard-gate` for an ISO-track certificate standard draft with normative grammar and conformance vectors, rejecting an unsupported item; see [docs/iso-track-certificate-standard.md](docs/iso-track-certificate-standard.md).
Run `make certificate-interchange-language-gate` for **PLCI/1**, a certificate interchange language whose ABNF vectors agree across Go, Rust, Python, and TypeScript checkers; see [docs/certificate-interchange-language.md](docs/certificate-interchange-language.md).
Run `make certificate-migration-gate` for backward-compatible certificate migration proving legacy PLCI/0 verdicts re-render and recheck as canonical PLCI/1 certificates; see [docs/certificate-migration.md](docs/certificate-migration.md).

Run `make standards-body-conformance-corpus-gate` for a PLCI/1 standards-body conformance corpus where every certificate example has a positive proof, negative control, and signed reference output; see [docs/standards-body-conformance-corpus.md](docs/standards-body-conformance-corpus.md).
Run `make certificate-interop-dashboard-gate` for a certificate interop dashboard that reruns Go, Rust, Python, and TypeScript checkers on the frozen corpus and publishes signed-reference drift deltas; see [docs/certificate-interop-dashboard.md](docs/certificate-interop-dashboard.md).
Run `make conformance-failure-minimizer-gate` for a conformance failure minimizer that reduces a cross-checker PLCI/1 disagreement to one certificate witness plus reference/observed deltas; see [docs/conformance-failure-minimizer.md](docs/conformance-failure-minimizer.md).
Run `make cross-tool-verdict-exchange-gate` for **cross-tool proof-carrying verdict exchange** that round-trips PLCI/1 verdict projections across strong_migrations, Django, and Prisma; see [docs/verdict-exchange.md](docs/verdict-exchange.md).
Run `make multi-impl-conformance-gate` for a multi-implementation **conformance suite** proving independent certificate checkers agree, rejecting an unsupported item; see [docs/multi-impl-conformance.md](docs/multi-impl-conformance.md).
Run `make ide-native-integration-gate` for an **IDE-native integration** surfacing hazards and certificates inline as developers edit migrations, rejecting an unsupported item; see [docs/ide-native-integration.md](docs/ide-native-integration.md).
Run `make ci-marketplace-actions-gate` for published **CI marketplace** actions for the major platforms with pinned, reproducible behavior, rejecting an unsupported item; see [docs/ci-marketplace-actions.md](docs/ci-marketplace-actions.md).
Run `make native-orm-plugins-gate` for **native ORM plugins** wiring Patchline gates into each framework's official migration command, rejecting an unsupported item; see [docs/native-orm-plugins.md](docs/native-orm-plugins.md).
Run `make cloud-provider-integration-gate` for **cloud-provider integration**s gating managed-database schema changes before they apply, rejecting an unsupported item; see [docs/cloud-provider-integration.md](docs/cloud-provider-integration.md).
Run `make registry-distribution-gate` for reproducible package-**registry distribution** with signed, content-pinned releases, rejecting an unsupported item; see [docs/registry-distribution.md](docs/registry-distribution.md).
Run `make enterprise-sso-rbac-gate` for enterprise **SSO and RBAC** with audited, least-privilege access to analyses and certificates, rejecting an unsupported item; see [docs/enterprise-sso-rbac.md](docs/enterprise-sso-rbac.md).
Run `make compliance-evidence-export-gate` for a compliance evidence export mapping gate results to common audit frameworks, rejecting an unsupported item; see [docs/compliance-evidence-export.md](docs/compliance-evidence-export.md).
Run `make multi-region-hosted-slo-gate` for a multi-region hosted service meeting a published **multi-region SLO** with failover evidence, rejecting an unsupported item; see [docs/multi-region-hosted-slo.md](docs/multi-region-hosted-slo.md).
Run `make neutral-governance-foundation-gate` for a neutral governance foundation charter with named members and open decision records, rejecting an unsupported item; see [docs/neutral-governance-foundation.md](docs/neutral-governance-foundation.md).
Run `make partner-certification-program-gate` for a **partner certification** program with conformance tests and a public registry of passers, rejecting an unsupported item; see [docs/partner-certification-program.md](docs/partner-certification-program.md).
Run `make university-curriculum-network-gate` for a university **curriculum network** adopting gate-backed assignments across multiple courses, rejecting an unsupported item; see [docs/university-curriculum-network.md](docs/university-curriculum-network.md).
Run `make standards-interop-plugfest-gate` for a standards **interop plugfest** where independent tools exchange and verify certificates, rejecting an unsupported item; see [docs/standards-interop-plugfest.md](docs/standards-interop-plugfest.md).
Run `make million-run-adoption-gate` for a **million-run adoption** milestone with a measured activation, retention, and expansion funnel, rejecting an unsupported item; see [docs/million-run-adoption.md](docs/million-run-adoption.md).
Run `make global-suite-repair-synthesis-gate` for a global-suite repair synthesis fixing an entire migration history to a whole-schema invariant set, rejecting an unsupported item; see [docs/global-suite-repair-synthesis.md](docs/global-suite-repair-synthesis.md).
Run `make verified-learned-heuristics-gate` for **verified learned heuristics** whose acceptance never weakens the underlying soundness guarantee, rejecting an unsupported item; see [docs/verified-learned-heuristics.md](docs/verified-learned-heuristics.md).
Run `make neuro-symbolic-proof-search-gate` for a **neuro-symbolic proof search** whose every proposed proof is independently machine-checked, rejecting an unsupported item; see [docs/neuro-symbolic-proof-search.md](docs/neuro-symbolic-proof-search.md).
Run `make schema-foundation-model-gate` for a **schema foundation model** with a downstream hazard-accuracy gate and deterministic verification, rejecting an unsupported item; see [docs/schema-foundation-model.md](docs/schema-foundation-model.md).
Run `make hazard-taxonomy-completeness-gate` for a hazard-**taxonomy completeness** argument with a canonical-form algorithm and coverage proof, rejecting an unsupported item; see [docs/hazard-taxonomy-completeness.md](docs/hazard-taxonomy-completeness.md).
Run `make information-bottleneck-features-gate` for an information-bottleneck feature study bounding minimal sufficient features per hazard class, rejecting an unsupported item; see [docs/information-bottleneck-features.md](docs/information-bottleneck-features.md).
Run `make automated-lemma-library-gate` for an **automated lemma library** that discovers, proves, and reuses safety lemmas across analyses, rejecting an unsupported item; see [docs/automated-lemma-library.md](docs/automated-lemma-library.md).
Run `make streaming-state-transfer-repair-gate` for a streaming state-transfer repair extending hazards to event-sourced and CRDT transitions, rejecting an unsupported item; see [docs/streaming-state-transfer-repair.md](docs/streaming-state-transfer-repair.md).
Run `make verified-macro-extension-gate` for a **verified macro extension** suite with hygiene proofs so user extensions cannot break soundness, rejecting an unsupported item; see [docs/verified-macro-extension.md](docs/verified-macro-extension.md).
Run `make live-benchmark-anti-overfit-gate` for a live, growing benchmark with an **anti-overfitting audit** and held-out refresh schedule, rejecting an unsupported item; see [docs/live-benchmark-anti-overfit.md](docs/live-benchmark-anti-overfit.md).
Run `make mechanistic-decision-circuits-gate` for a mechanistic study extracting the **decision circuits** that drive each learned verdict, rejecting an unsupported item; see [docs/mechanistic-decision-circuits.md](docs/mechanistic-decision-circuits.md).
...
Run `make interactive-claim-explorer-gate` for an **interactive claim explorer** letting readers re-run any claim against live gates, rejecting an unsupported item; see [docs/interactive-claim-explorer.md](docs/interactive-claim-explorer.md).
Run `make doi-snapshot-attestation-2-gate` for a DOI-pinned snapshot with a bit-identical rebuild attestation and frozen dependency closure, rejecting an unsupported item; see [docs/doi-snapshot-attestation-2.md](docs/doi-snapshot-attestation-2.md).
Run `make threats-validity-matrix-2-gate` for a **threats-to-validity matrix** mapping every threat to a robustness or ablation experiment, rejecting an unsupported item; see [docs/threats-validity-matrix-2.md](docs/threats-validity-matrix-2.md).
Run `make living-related-work-comparison-gate` for a **living related-work** comparison regenerated from a shared, frozen benchmark harness, rejecting an unsupported item; see [docs/living-related-work-comparison.md](docs/living-related-work-comparison.md).
Run `make negative-results-registry-gate` for a **negative-results registry** with a reproducible experiment for every claimed boundary, rejecting an unsupported item; see [docs/negative-results-registry.md](docs/negative-results-registry.md).
Run `make external-artifact-audit-2-gate` for an **external artifact audit** dry run with a reviewer's independent reproduction log, rejecting an unsupported item; see [docs/external-artifact-audit-2.md](docs/external-artifact-audit-2.md).
Run `make results-never-regress-2-gate` for a **never-regress guarantee** enforced by the full historical benchmark on every release, rejecting an unsupported item; see [docs/results-never-regress-2.md](docs/results-never-regress-2.md).
Run `make multi-org-incident-reduction-2-gate` for an independently-audited **multi-org incident reduction** with pooled, pre-registered analysis, rejecting an unsupported item; see [docs/multi-org-incident-reduction-2.md](docs/multi-org-incident-reduction-2.md).
Run `make impact-citation-graph-gate` for an impact **citation graph** tying adoption and citations to the DOI-pinned artifact, rejecting an unsupported item; see [docs/impact-citation-graph.md](docs/impact-citation-graph.md).
Run `make successor-frontier-roadmap-gate` for a successor-frontier roadmap proving the next hazard frontier is reachable without a rewrite, rejecting an unsupported item; see [docs/successor-frontier-roadmap.md](docs/successor-frontier-roadmap.md).
Run `make funded-sustainability-trust-gate` for a funded sustainability trust with a published, multi-year maintenance budget, rejecting an unsupported item; see [docs/funded-sustainability-trust.md](docs/funded-sustainability-trust.md).
Run `make governance-bus-factor-proof-gate` for a governance **bus-factor proof** with documented succession and term limits, rejecting an unsupported item; see [docs/governance-bus-factor-proof.md](docs/governance-bus-factor-proof.md).
Run `make ratified-standard-conformance-gate` for a ratified-standard conformance result with reference implementations passing all vectors, rejecting an unsupported item; see [docs/ratified-standard-conformance.md](docs/ratified-standard-conformance.md).
Run `make award-readiness-dossier-2-gate` for an **award-readiness dossier** scored against published best-paper and best-artifact rubrics, rejecting an unsupported item; see [docs/award-readiness-dossier-2.md](docs/award-readiness-dossier-2.md).
Run `make ten-thousand-star-funnel-gate` for a ten-thousand-star growth result with a reproducible, measured acquisition funnel, rejecting an unsupported item; see [docs/ten-thousand-star-funnel.md](docs/ten-thousand-star-funnel.md).
Run `make decade-impact-retrospective-gate` for a **decade impact retrospective** tying every major decision to a measured, published outcome, rejecting an unsupported item; see [docs/decade-impact-retrospective.md](docs/decade-impact-retrospective.md).
Run `make grand-unified-evidence-index-2-gate` for a **grand-unified evidence index** proving formal core, scale, causal rigor, autonomy, adoption, and reproducibility in one command, rejecting an unsupported item; see [docs/grand-unified-evidence-index-2.md](docs/grand-unified-evidence-index-2.md).
